...
class IT_RST_RP_Menu {
    ...
    public static function get_menu_data( $request ) {

		global $wpdb, $it_rst_reporting_feature_settings, $it_rst_dashboard_settings;
		...
		if ( $user_obj ) {
		    ...
		    $box_coord = json_decode( get_user_meta( $user_id, 'it_rooster_box_coord', TRUE ) );
		    
		    if ( !is_array( $box_coord ) ) {
			    $box_coord = array();
		    }
		    ...
		    foreach ( $it_rst_reporting_feature_settings as $k => $v ) {
			    ...
			    foreach ( $box_coord as $box ) {
				    if ( isset( $box->slug ) && $box->slug == $k ) {
					    $it_rst_reporting_feature_settings[ $k ]['coordinates'] = array( 'x' => $box->x, 'y' => $box->y, 'width' => $box->width, 'height' => $box->height );
					    break;
				    }
		    	} 
		    }
